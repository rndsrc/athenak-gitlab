...
//! \brief derived class that implements isothermal EOS in general relativistic hydro
...
IsothermalGRHydro::IsothermalGRHydro(MeshBlockPack *pp, ParameterInput *pin) :
    EquationOfState("hydro", pp, pin) {
  eos_data.is_ideal = false;
  eos_data.gamma = 0.0;
  eos_data.iso_cs = pin->GetReal("hydro","iso_sound_speed");
  eos_data.iso_cs_rel_lim = pin->GetReal("hydro","iso_sound_speed_limit");
...
void IsothermalGRHydro::ConsToPrim(DvceArray5D<Real> &cons, DvceArray5D<Real> &prim,
  ...
  KOKKOS_LAMBDA(const int &idx, int &sumd, int &sumv, int &sumf, int &max_it) {
    ...
    if (!(excised)) {
      ...
      // call c2p function
      // (inline function in isothermal_c2p_hyd.hpp file)
      SingleC2P_IsothermalSRHyd(u_sr, eos, s2, w,
    ...
    if (only_testfloors) {
      ...
      if (dfloor_used ||  vceiling_used || c2p_failure || excised) {
        SingleP2C_IsothermalGRHyd(glower, gupper, w, eos.iso_cs, eos.iso_cs_rel_lim, u);
...
void IsothermalGRHydro::PrimToCons(const DvceArray5D<Real> &prim, DvceArray5D<Real> &cons,
                              const int il, const int iu, const int jl, const int ju,
                              const int kl, const int ku) {
  auto &indcs = pmy_pack->pmesh->mb_indcs;
  int &is = indcs.is, &js = indcs.js, &ks = indcs.ks;
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    // call p2c function
    HydCons1D u;
    SingleP2C_IsothermalGRHyd(glower, gupper, w, iso_cs, iso_cs_rel_lim, u);
