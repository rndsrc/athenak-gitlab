...
void RadiationMoments::ConsToPrim(DvceArray5D<Real> &cons, DvceArray5D<Real> &prim)
{
  ...
  auto moments_coord_ = pmy_pack->prad->moments_coord;
  ...
  // Zero moments
  par_for("zero_moments", DevExeSpace(),0,(nmb-1),0,10,ks,ke,js,je,is,ie,
    KOKKOS_LAMBDA(int m, int n, int k, int j, int i)
    {
      moments_coord_(m,n,k,j,i) = 0.0;
    }
  );
      ...
      for (int n1 = 0, n12 = 0; n1 < 4; ++n1) {
        for (int n2 = n1; n2 < 4; ++n2, ++n12) {
          for (int k = ks; k <= ke; ++k) {
            for (int j = js; j <= je; ++j) {
              for (int i = is; i <= ie; ++i) {
                moments_coord_(m,n12,k,j,i) += nmu_(m,n1,z,p,k,j,i) * nmu_(m,n2,z,p,k,j,i)
