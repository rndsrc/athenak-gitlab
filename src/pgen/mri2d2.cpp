...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  int iperturbation = pin->GetOrAddInteger("problem","iperturbation",1);
  int inumber = pin->GetOrAddInteger("problem","inumber",40);
  ...
  Real ky = 2.0*(M_PI/x2size)*(static_cast<Real>(nwx));

  // change these to dual arrays, access kxs.h_view etc...
  DualArray1D<double> kxs("kxs",inumber);
  DualArray1D<double> kys("kys",inumber);
  DualArray1D<double> phases("phases",inumber);
  
  if (iperturbation == 2){
    //std::random_device rd;  // Will be used to obtain a seed for the random number engine
    std::mt19937 gen(0);//(rd()); // Standard mersenne_twister_engine seeded with rd(); or use zero as a seed to give consistent results
    std::uniform_int_distribution<> uniform(-5,5);//(-10*(M_PI/x1size)*(static_cast<Real>(nwx)), 10*(M_PI/x1size)*(static_cast<Real>(nwx)));
    //std::uniform_real_distribution<> uniform_2(-10*(M_PI/x1size)*(static_cast<Real>(nwx)), 10*(M_PI/x2size)*(static_cast<Real>(nwx)));
    std::uniform_real_distribution<> uni_phase(-M_PI,M_PI);
    for (int n = 0; n < inumber; ++n) {
      // Use uniform to transform the random unsigned int generated by gen into a 
      // double in [1, 2). Each call to uniform(gen) generates a new random double
      kxs.h_view(n) = uniform(gen)*(2*M_PI/x1size);
      kys.h_view(n) = uniform(gen)*(2*M_PI/x2size);
      phases.h_view(n) = uni_phase(gen);
      //std::cout << dis(gen) << ' ';
    }
  }
  kxs.template modify<HostMemSpace>();
  kxs.template sync<DevExeSpace>();
  kys.template modify<HostMemSpace>();
  kys.template sync<DevExeSpace>();
  phases.template modify<HostMemSpace>();
  phases.template sync<DevExeSpace>();
  // need to create copies of data on device and host, can only call std functions on 
  // the host (cpu); need kokkos dual array. create dual view, initialize the host data, 
  // call kokkos function to sync it. 
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      if (ifield == 1) {
        ...
        if (k==ke) b0.x3f(m,ke+1,j,i) = 0.0;
          ...
          b0.x1f(m,k,j,i) = 0.0;
          if (x1v>=0){b0.x2f(m,k,j,i) = B0;}
            else if (x1v<0){b0.x2f(m,k,j,i) = -B0;}
          b0.x3f(m,k,j,i) = 0.0;
          if (i==ie) b0.x1f(m,k,j,ie+1) = 0.0;
          if (j==je) {if (x1v>=0){b0.x2f(m,k,je+1,i) = B0;}
            else if (x1v<0){b0.x2f(m,k,je+1,i) = -B0;};}
          if (k==ke) b0.x3f(m,ke+1,j,i) = 0.0;
      } else if (ifield == 4) {
          b0.x1f(m,k,j,i) = 0.0;
          b0.x3f(m,k,j,i) = 0.0;
          ...
          if (x1v>=0.25){b0.x2f(m,k,j,i) = B0*(-x1v*4+2);
          } else if (x1v<=-0.25){b0.x2f(m,k,j,i) = B0*(-x1v*4-2);
          } else {b0.x2f(m,k,j,i) = B0*(x1v*4);}

          if (j==je){
            if (x1v>=0.25){b0.x2f(m,k,je+1,i) = B0*(-x1v*4+2);
            } else if (x1v<=-0.25){b0.x2f(m,k,je+1,i) = B0*(-x1v*4-2);
            } else {b0.x2f(m,k,je+1,i) = B0*(x1v*4);}
          } 
      }
    });
  }
  ...
  if (pmbp->pmhd != nullptr) {
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      //access my array here, use .d_view (d for device)
      ...
      if (eos.is_ideal) {
        if(iperturbation ==1){
          ...
          for (int n = 0; n < inumber; ++n) {
            Real kx1 = kxs.d_view(n);
            Real ky1 = kys.d_view(n);
            Real phase1 = phases.d_view(n);
            u0(m,IEN,k,j,i) += amp*p0/gm1*sin(kx1*x1v+ky1*x2v+phase1);// *(1+cos(kx/2*x1v))*(1+cos(kx/2*x2v));
          }
        }
      }
