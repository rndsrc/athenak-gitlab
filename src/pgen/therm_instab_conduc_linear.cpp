...
//========================================================================================
//! \file therm_instab_conduc_linear.cpp
...
void ProblemGenerator::UserProblem(MeshBlockPack *pmbp, ParameterInput *pin)
{
  ...
  if (pmbp->phydro != nullptr) {
    ...
    par_for("pgen_therm_instab_conduc_linear", DevExeSpace(),0,nmb1,ks,ke,js,je,is,ie,
...
static Real ThermalInstabilityGrowthRate(const Real rho, const Real pgas, 
  ...
  if(kappa_iso > 0.0){
      k2_over_k_kappa = k*k/(pgas*cs/gm1/(temp/punit->temperature)/kappa_iso);
  } else {
      k2_over_k_kappa = 0.0;
  }
