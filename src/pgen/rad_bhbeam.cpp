...
//========================================================================================
//! \file rad_bhbeam.cpp
//  \brief Beam test for radiation around Kerr black hole
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    ComputeMetricAndInverse(x1, x2, x3, coord.is_minkowski, coord.bh_spin, g_, gi_);
    Real e[4][4] = {0.0}; Real e_cov[4][4] = {0.0}; Real omega[4][4][4] = {0.0};
    ComputeTetrad(x1, x2, x3, coord.is_minkowski, coord.bh_spin, e, e_cov, omega);
    ...
    Real d1 = -x2/sqrt(SQR(x1)+SQR(x2));
    Real d2 =  x1/sqrt(SQR(x1)+SQR(x2));
    Real d3 = 0.0;
    d1 /= sqrt(SQR(fabs(x1/sqrt(SQR(x1)+SQR(x2)))) + SQR(fabs(x1/sqrt(SQR(x1)+SQR(x2)))));
    d2 /= sqrt(SQR(fabs(x1/sqrt(SQR(x1)+SQR(x2)))) + SQR(fabs(x1/sqrt(SQR(x1)+SQR(x2)))));
    d3 /= sqrt(SQR(fabs(x1/sqrt(SQR(x1)+SQR(x2)))) + SQR(fabs(x1/sqrt(SQR(x1)+SQR(x2)))));
    Real temp_a = g_[I00];
