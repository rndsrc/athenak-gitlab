   ...
   out_params(opar)
{
  ...
  switch (out_params.variable) {
    ...
    // Load radiation conserved variables
    case OutputVariable::rad_ci_0:
      if (pm->pmb_pack->prad == nullptr) ErrHydroOutput(out_params.block_name);
      outvars.emplace_back("ci",0,&(pm->pmb_pack->prad->ci0));
      break;

    // Load radiation primitive variables
    case OutputVariable::rad_i_0:
      if (pm->pmb_pack->prad == nullptr) ErrHydroOutput(out_params.block_name);
      outvars.emplace_back("i",0,&(pm->pmb_pack->prad->i0));
      break;
