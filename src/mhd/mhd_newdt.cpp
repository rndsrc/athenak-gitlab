...
TaskStatus MHD::NewTimeStep(Driver *pdriver, int stage)
{
  ...
  if (pdriver->time_evolution == TimeEvolution::kinematic) {
	...
	Real &w_vx = w0_(m,IVX,k,j,i);
	Real &w_vy = w0_(m,IVY,k,j,i);
	Real &w_vz = w0_(m,IVZ,k,j,i);
	...
	Real b0,u2,u0,b2,wgas,p,lm,lp;
	{

	  b0 = w_bx * w_vx + w_by * w_vy + w_bz * w_vz;
	  u2 = SQR(w_vz) + SQR(w_vy) + SQR(w_vx);
	  u0  = sqrt(1. + u2);
	  b2 = ((w_bx*w_bx + w_by*w_by + w_bz*w_bz) + b0*b0) / (1.+u2);
	  // FIXME ERM: Ideal fluid for now
	  wgas = w_d + (eos.gamma/gm1) * w_p;
	  p = w_p + 0.5*b2;

	  eos.FastMagnetosonicSpeedSR(wgas,b2, w_p, w_vx/u0, 1.+u2, lp, lm);
	  min_dt1 = fmin((mbsize.dx1.d_view(m)/(fmax(fabs(lm),lp))), min_dt1);
        }
	...
	w_bz = bcc0_(m,IBX,k,j,i);
	{
	  ...
	  eos.FastMagnetosonicSpeedSR(wgas,b2, w_p, w_vy/u0, 1.+u2, lp, lm);
	  min_dt2 = fmin((mbsize.dx2.d_view(m)/(fmax(fabs(lm),lp))), min_dt2);
        }
	...
	w_bz = bcc0_(m,IBY,k,j,i);
	{
	  ...
	  eos.FastMagnetosonicSpeedSR(wgas,b2, w_p, w_vz/u0, 1.+u2, lp, lm);
	  min_dt3 = fmin((mbsize.dx3.d_view(m)/(fmax(fabs(lm),lp))), min_dt3);
        }
