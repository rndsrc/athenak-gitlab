...
class GeodesicGrid {
 public:
  GeodesicGrid(int nlev, bool rotate, bool fluxes, Real rad);
  ~GeodesicGrid();

  int nangles;  // number of angles (derived from nlevel, 10*nlevel^2 + 2)
  Real radius;  // radius of geodesic mesh (==1 is unit sphere)
  ...
  DualArray2D<Real> cart_pos;        // coordinate position (cartesian) at face center
  DualArray3D<Real> cart_pos_mid;    // coordinate position (cartesian) at face edges
