  ...
  if (nlevel > 0) {  // construct geodesic mesh
    ...
    if (geo_fluxes) {
      if (rotate_geo) {
        ...
        // reallocate ximn, etamn arrays
        Kokkos::realloc(xi_eta, nangles, 6, 2);

        // set xi/eta grid positions for MAPR reconstruction
        for (int n=0; n<nangles; ++n) {
          Real xi_coord[6]; Real eta_coord[6];
          ComputeXiEta(n, xi_coord, eta_coord);
          for (int nb=0; nb<6; ++nb) {
            xi_eta.h_view(n,nb,0) = xi_coord[nb];
            xi_eta.h_view(n,nb,1) = eta_coord[nb];
          }
        }
      } else {
    ...
    xi_eta.template sync<DevExeSpace>();
...
//----------------------------------------------------------------------------------------
//! \fn void GeodesicGrid::ComputeXiEta
//! \brief assign xi, eta coordinates used by MAPR reconstruction

void GeodesicGrid::ComputeXiEta(int n, Real xi[6], Real eta[6]) {
  Real x0, y0, z0;
  GridCartPosition(n,x0,y0,z0);
  int nn; int nvec[6];
  Neighbors(n, nn, nvec);
  ...
  for (int nb=0; nb<nn; ++nb) {
    Real xn1, yn1, zn1;
    Real xn2, yn2, zn2;
    GridCartPosition(nvec[nb],       xn1,yn1,zn1);
    GridCartPosition(nvec[(nb+1)%nn],xn2,yn2,zn2);
