...
GeodesicGrid::GeodesicGrid(int nlev, bool rotate, bool fluxes, Real rad) :
    nlevel(nlev),
    rotate_geo(rotate),
    geo_fluxes(fluxes),
    radius(rad),
  ...
  if (nlevel > 0) {  // construct geodesic mesh
    ...
    // set poles explicitly
    apnorm.h_view(0,0) = 0.0;
    apnorm.h_view(0,1) = 0.0;
    apnorm.h_view(0,2) = 1.0;
    apnorm.h_view(1,0) = 0.0;
    apnorm.h_view(1,1) = 0.0;
    apnorm.h_view(1,2) = -1.0;
    ...
    for (int l=0; l<nlevel; ++l) {
      ...
      for (int m=l; m<nlevel; ++m) {
        ...
        anorm.h_view(0,row_index,col_index,0) = x/norm;
        anorm.h_view(0,row_index,col_index,1) = y/norm;
        anorm.h_view(0,row_index,col_index,2) = z/norm;
        col_index += 1;
      }
    ...
    for (int ptch=1; ptch<5; ++ptch) {
      for (int l=1; l<1+nlevel; ++l) {
        for (int m=1; m<1+2*nlevel; ++m) {
          Real x0 = anorm.h_view(0,l,m,0);
          Real y0 = anorm.h_view(0,l,m,1);
          Real z0 = anorm.h_view(0,l,m,2);
          anorm.h_view(ptch,l,m,0) = (x0*cos(ptch*0.4*M_PI)+y0*sin(ptch*0.4*M_PI));
          anorm.h_view(ptch,l,m,1) = (y0*cos(ptch*0.4*M_PI)-x0*sin(ptch*0.4*M_PI));
          anorm.h_view(ptch,l,m,2) = z0;
        }
      }
    }
    ...
    for (int i=0; i<3; ++i) {
      for (int bl=0; bl<5; ++bl) {
        for (int k=0; k<nlevel; ++k) {
          ...
          anorm.h_view(bl,0,k+nlevel+1,i)  =anorm.h_view((bl+4)%5,nlevel,k+1,i);
          anorm.h_view(bl,k+1,2*nlevel+1,i)=anorm.h_view((bl+4)%5,nlevel,k+nlevel+1,i);
          anorm.h_view(bl,k+2,0,i)         =anorm.h_view((bl+1)%5,1,k+1,i);
          anorm.h_view(bl,nlevel+1,k+1,i)  =anorm.h_view((bl+1)%5,1,k+nlevel+1,i);
          anorm.h_view(bl,nlevel+1,k+nlevel+1,i)=anorm.h_view((bl+1)%5,k+2,2*nlevel,i);
        }
        anorm.h_view(bl,1,0,i) = apnorm.h_view(0,i);
        anorm.h_view(bl,nlevel+1,2*nlevel,i) = apnorm.h_view(1,i);
        anorm.h_view(bl,0,2*nlevel+1,i) = anorm.h_view(bl,0,2*nlevel,i);
      }
    }
    ...
    apind.h_view(0) = 5*2*SQR(nlevel);
    apind.h_view(1) = 5*2*SQR(nlevel) + 1;
    for (int ptch=0; ptch<5; ++ptch) {
      for (int l=0; l<nlevel; ++l) {
        for (int m=0; m<2*nlevel; ++m) {
          aind.h_view(ptch,l+1,m+1) = ptch*2*SQR(nlevel) + l*2*nlevel + m;
        }
      }
    }
    ...
    for (int bl=0; bl<5; ++bl) {
      for (int k=0; k<nlevel; ++k) {
        aind.h_view(bl,0,k+1)               = aind.h_view((bl+4)%5,k+1,1);
        aind.h_view(bl,0,k+nlevel+1)        = aind.h_view((bl+4)%5,nlevel,k+1);
        aind.h_view(bl,k+1,2*nlevel+1)      = aind.h_view((bl+4)%5,nlevel,k+nlevel+1);
        aind.h_view(bl,k+2,0)               = aind.h_view((bl+1)%5,1,k+1);
        aind.h_view(bl,nlevel+1,k+1)        = aind.h_view((bl+1)%5,1,k+nlevel+1);
        aind.h_view(bl,nlevel+1,k+nlevel+1) = aind.h_view((bl+1)%5,k+2,2*nlevel);
      }
      aind.h_view(bl,1,0) = apind.h_view(0);
      aind.h_view(bl,nlevel+1,2*nlevel) = apind.h_view(1);
      aind.h_view(bl,0,2*nlevel+1) = aind.h_view(bl,0,2*nlevel);
    }
    ...
    // correct for round-off error level differences in arc lengths among shared edges
    for (int n=0; n<nangles; ++n) {
      for (int nb=0; nb<numn.h_view(n); ++nb) {
        for (int nnb=0; nnb<numn.h_view(indn.h_view(n,nb)); ++nnb) {
          if (n==indn.h_view(indn.h_view(n,nb),nnb)) {
            Real arc_avg = 0.5*(arcl.h_view(n,nb) + arcl.h_view(indn.h_view(n,nb),nnb));
            arcl.h_view(n,nb) = arc_avg;
            arcl.h_view(indn.h_view(n,nb),nnb) = arc_avg;
          }
        }
      }
    }
    ...
    if (geo_fluxes) {
      if (rotate_geo) {
        ...
        Kokkos::realloc(unit_flux, nangles, 6, 3);
        ...
        for (int n=0; n<nangles; ++n) {
          for (int nb=0; nb<numn.h_view(n); ++nb) {
            ...
            for (int nnb=0; nnb<numn.h_view(indn.h_view(n,nb)); ++nnb) {
              if (n==indn.h_view(indn.h_view(n,nb),nnb)) {
                Real nuzeta = unit_flux.h_view(indn.h_view(n,nb),nnb,0);
                Real nupsi  = unit_flux.h_view(indn.h_view(n,nb),nnb,1);
                ...
                unit_flux.h_view(indn.h_view(n,nb),nnb,0) = copysign(uzeta_avg, nuzeta);
                unit_flux.h_view(indn.h_view(n,nb),nnb,1) = copysign(upsi_avg, nupsi);
              }
            }
          }
        }
        ...
        std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
          << std::endl << "Geodesic grid must be rotated if fluxes enabled" << std::endl;
        std::exit(EXIT_FAILURE);
      }
    }

    // set grid positions, weight by radius
    for (int n=0; n<nangles; ++n) {
      Real x, y, z;
      GridCartPosition(n,x,y,z);
      cart_pos.h_view(n,0) = radius*x;
      cart_pos.h_view(n,1) = radius*y;
      cart_pos.h_view(n,2) = radius*z;
      int nn = num_neighbors.h_view(n);
      for (int nb=0; nb<nn; ++nb) {
        Real xm, ym, zm;
        GridCartPositionMid(n,ind_neighbors.h_view(n,nb),xm,ym,zm);
        cart_pos_mid.h_view(n,nb,0) = radius*xm;
        cart_pos_mid.h_view(n,nb,1) = radius*ym;
        cart_pos_mid.h_view(n,nb,2) = radius*zm;
      }
      if (nn==5) {
    ...
    // sync dual arrays
    anorm.template modify<HostMemSpace>();
    anorm.template sync<DevExeSpace>();
    apnorm.template modify<HostMemSpace>();
    apnorm.template sync<DevExeSpace>();
    aind.template modify<HostMemSpace>();
    aind.template sync<DevExeSpace>();
    apind.template modify<HostMemSpace>();
    apind.template sync<DevExeSpace>();
    ...
    // error if fluxes enabled or rotating mesh
    ...
    for (int z=0, n=0; z<2; ++z) {
      for (int p=0; p<4; ++p, ++n) {
        ...
        cart_pos.h_view(n,0) = rad*sin(zetav[z])*cos(psiv[p])*sqrt(4.0/3.0);
        cart_pos.h_view(n,1) = rad*sin(zetav[z])*sin(psiv[p])*sqrt(4.0/3.0);
        cart_pos.h_view(n,2) = rad*cos(zetav[z])*sqrt(2.0/3.0);
      }
    }
...
void GeodesicGrid::GridCartPosition(int n, Real& x, Real& y, Real& z) {
  ...
  if (ibl0 == 5) {
    x = ameshp_normals.h_view(ibl2, 0);
    y = ameshp_normals.h_view(ibl2, 1);
    z = ameshp_normals.h_view(ibl2, 2);
  } else {
    x = amesh_normals.h_view(ibl0,ibl1+1,ibl2+1,0);
    y = amesh_normals.h_view(ibl0,ibl1+1,ibl2+1,1);
    z = amesh_normals.h_view(ibl0,ibl1+1,ibl2+1,2);
  }
  return;
}
...
void GeodesicGrid::Neighbors(int n, int& num_nghbr, int neighbors[6]) {
  if (n==5*2*nlevel*nlevel) {  // handle north pole
    for (int bl=0; bl<5; ++bl) {
      neighbors[bl] = amesh_indices.h_view(bl,1,1);
    }
    neighbors[5] = (INT_MAX);
    num_nghbr = 5;
  } else if (n==5*2*nlevel*nlevel + 1) {  // handle south pole
    for (int bl=0; bl<5; ++bl) {
      neighbors[bl] = amesh_indices.h_view(bl,nlevel,2*nlevel);
    }
    neighbors[5] = (INT_MAX);
    num_nghbr = 5;
  } else {
    ...
    int ibl1 = (n % (2*nlevel*nlevel)) / (2*nlevel);
    int ibl2 = (n % (2*nlevel*nlevel)) % (2*nlevel);
    neighbors[0] = amesh_indices.h_view(ibl0,ibl1+1,ibl2+2);
    neighbors[1] = amesh_indices.h_view(ibl0,ibl1+2,ibl2+1);
    neighbors[2] = amesh_indices.h_view(ibl0,ibl1+2,ibl2  );
    neighbors[3] = amesh_indices.h_view(ibl0,ibl1+1,ibl2  );
    neighbors[4] = amesh_indices.h_view(ibl0,ibl1  ,ibl2+1);
    ...
    if (n % (2*nlevel*nlevel) == nlevel-1 ||
      ...
      neighbors[5] = amesh_indices.h_view(ibl0, ibl1, ibl2+2);
      num_nghbr = 6;
    }
  }
  return;
...
void GeodesicGrid::RotateGrid(Real zetanew, Real psinew) {
  Real kx = -sin(psinew);
  Real ky =  cos(psinew);
  auto &amn=amesh_normals;
  auto &ampn=ameshp_normals;
  Real vx, vy, vz, vrx, vry, vrz;
  for (int bl=0; bl<5; ++bl) {
    for (int l=0; l<nlevel; ++l) {
      for (int m=0; m<2*nlevel; ++m) {
        vx = amn.h_view(bl, l+1, m+1, 0);
        vy = amn.h_view(bl, l+1, m+1, 1);
        vz = amn.h_view(bl, l+1, m+1, 2);
        vrx = (vx*cos(zetanew)+ky*vz*sin(zetanew) + kx*(kx*vx+ky*vy)*(1.0-cos(zetanew)));
        vry = (vy*cos(zetanew)-kx*vz*sin(zetanew) + ky*(kx*vx+ky*vy)*(1.0-cos(zetanew)));
        vrz = (vz*cos(zetanew)+(kx*vy-ky*vx)*sin(zetanew));
        amn.h_view(bl,l+1,m+1,0) = vrx;
        amn.h_view(bl,l+1,m+1,1) = vry;
        amn.h_view(bl,l+1,m+1,2) = vrz;
      }
    }
  }
  for (int pl=0; pl<2; ++pl) {
    ...
    vry = vy*cos(zetanew)-kx*vz*sin(zetanew)+ky*(kx*vx+ky*vy)*(1.0-cos(zetanew));
    vrz = vz*cos(zetanew)+(kx*vy-ky*vx)*sin(zetanew);
    ampn.h_view(pl,0) = vrx;
    ampn.h_view(pl,1) = vry;
    ampn.h_view(pl,2) = vrz;
  }
  for (int i=0; i<3; ++i) {
    for (int bl=0; bl<5; ++bl) {
      for (int k=0; k<nlevel; ++k) {
        amn.h_view(bl,0,k+1,i)               = amn.h_view((bl+4)%5,k+1,1,i);
        amn.h_view(bl,0,k+nlevel+1,i)        = amn.h_view((bl+4)%5,nlevel,k+1,i);
        amn.h_view(bl,k+1,2*nlevel+1,i)      = amn.h_view((bl+4)%5,nlevel,k+nlevel+1,i);
        amn.h_view(bl,k+2,0,i)               = amn.h_view((bl+1)%5,1,k+1,i);
        amn.h_view(bl,nlevel+1,k+1,i)        = amn.h_view((bl+1)%5,1,k+nlevel+1,i);
        amn.h_view(bl,nlevel+1,k+nlevel+1,i) = amn.h_view((bl+1)%5,k+2,2*nlevel,i);
      }
      amn.h_view(bl,1,0,i) = ampn.h_view(0,i);
      amn.h_view(bl,nlevel+1,2*nlevel,i) = ampn.h_view(1,i);
      amn.h_view(bl,0,2*nlevel+1,i) = amn.h_view(bl,0,2*nlevel,i);
    }
  }
  return;
}
...
//----------------------------------------------------------------------------------------
//! \fn void GeodesicGrid::RotateGrid
//! \brief find components of unit vectors along edges of angle centers

void GeodesicGrid::UnitFluxDir(Real zetav, Real psiv, Real zetaf, Real psif,
                               Real& dzeta, Real& dpsi) {
  if (fabs(psif-psiv) < 1.0e-10 || fabs(fabs(cos(zetav))-1) < 1.0e-10) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
        << std::endl << "Pole encountered in geodesic mesh" << std::endl;
    std::exit(EXIT_FAILURE);
  } else {
    Real a_par, p_par;
    ...
    Real denom = 1.0/sqrt(zeta_deriv*zeta_deriv+SQR(sin(zetaf)));
